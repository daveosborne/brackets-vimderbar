...
(function() {
  ...
  var defaultKeymap = [
    ...
    // Reverse find ("?") functionality removed by @ff. need to respect Brackets' native search
    { keys: ['*'], type: 'search',
  ...
  var Vim = function() {
    ...
    var motions = {
      ...
      findNext: function(cm, motionArgs) {
        if (CodeMirror.processVimSearch) { //if added by @ff
        	CodeMirror.processVimSearch(!motionArgs.forward, motionArgs.repeat);
        	return;
      	}
    ...
    function dialog(cm, template, shortText, onClose, options) {
     /*if (cm.openDialog) {
      ...
      if (CodeMirror.openVimDialog) {
        CodeMirror.openVimDialog(template, shortText, onClose, { bottom: true, value: options }, cm);
        // changed cm to CodeMirror. not sure what else to do atm, @ff.
      }
      else {
        onClose(prompt(shortText, ""));
        // old Vim.js used callback(prompt(shortText, "")); @ff
      }
    }
    ...
    function showConfirm(cm, text) {
      /*if (cm.openConfirm) {
        cm.openConfirm('<span style="color: red">' + text +
            ...
            {bottom: true});*/
      if (CodeMirror.openVimConfirm) {
        CodeMirror.openVimConfirm(text, function() {}, {bottom: true}, cm);
        // changed cm to CodeMirror, don't know how else to do it. -@ff
      } else {
        alert(text);
      }
    }
    function hideConfirm(cm) { // function added by @ff
      if (CodeMirror.hideVimConfirm) {
        CodeMirror.hideVimConfirm(function() {}, {bottom: true}, cm);
      }
    }
        ...
        if (input !== "") { // if added by @ff
          ...
          if (!params.commandName) {
            ...
            if (command) {
              ...
              hideConfirm(function() {}, cm); // added by @ff
            }
          }
          if (!commandName) {
            showConfirm(cm, 'Not an editor command: ' + input); //edited by @ff
            return;
          }
          try {
